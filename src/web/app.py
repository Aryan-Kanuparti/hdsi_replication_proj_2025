...
def display_educational_features(edu_data: dict):
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["🔍 How I Got This", "📚 Vocabulary", "⚠️ Limitations", "🎯 Next Steps"]
    )
    ...
    with tab1:
        ...
        if edu_data.get("decomposition"):
            ...
            for step in edu_data["decomposition"]:
                with st.expander(
                    f"Step {step.get('step', '')}: {step.get('question', '')}"
                ):
        ...
        if conf >= 80:
            st.caption(
                "🟢 High confidence - Query returned results with clear entity matches"
            )
        elif conf >= 60:
